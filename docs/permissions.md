...
| `GET /api/activities/` | 登录用户可访问；非已审批医生只看 active 活动，已审批医生可看全部 |
| `POST /api/activities/<pk>/join/` | 登录用户可报名；非已审批医生传 `doctor` 会被降级为 `volunteer` |
...
- 管理员页面需要管理员身份。
- 医生页面需要已审批医生身份；待审批医生只能看到等待审批页。
- 患者页面需要已审批患者身份；待审批患者只能看到等待审批页。
...
- 待审批医生不能直接访问医生 Web 页面
- 待审批医生 token 不能获得活动医生权限
- 管理端审批按钮使用 POST 表单而不是 GET 链接
- 医生 Web 端可通过表单确认病历并写入确认时间
- 患者只能查看自己的病历
